...
mystore::Client::get(std::string key, int numPastMappings) {
    int nextRequestIdentifier = this->getNextRequestIdentifier();

    return getHelper(key, "", nextRequestIdentifier, 0);
}
...
std::vector<std::string>
mystore::Client::getHelper(std::string key, std::string clientIdentifier, int requestIdentifier, int numPastMappings) {
        ...
        for(int retryNum = 0; retryNum < numRetriesBeforeChangingRequestID; ++retryNum) {
            ...
            try {
                ...
                try {
                    ...
                    client.get(getResponse, key, clientIdentifier, currentRequestNumber, 0);
...
bool
mystore::Client::delKey(std::string key) {
    int nextRequestIdentifier = this->getNextRequestIdentifier();

    return this->delKeyHelper(key, "", nextRequestIdentifier);
}

bool
mystore::Client::delKeyHelper(std::string key, std::string clientIdentifier, int requestIdentifier) {
    return false;
    int numCyclesBeforeQuitting =
            atoi(dotenv::env[mystore::Client::NUM_RETRY_CYCLES_BEFORE_QUITTING_ENV_VAR_NAME].c_str());
    int numRetriesBeforeChangingRequestID =
        ...
        for(int retryNum = 0; retryNum < numRetriesBeforeChangingRequestID; ++retryNum) {
            ...
            try {
                ...
                try {
                    ...
                    client.deletekey(delResponse, key, clientIdentifier, currentRequestNumber);
                    ...
                    else if(mystore::Client::isNullID(delResponse.leaderID)) {
                        std::pair<std::string, int> leaderInfo = this->getRandomReplica();
                        host = leaderInfo.first;
                        port = leaderInfo.second;
                    }
                    else {
                        host = delResponse.leaderID.hostname;
                        port = delResponse.leaderID.port;
                    }
                }
                catch(TTransportException& e) {
                }
    ...
    throw mystore::DelFailureException();
}
